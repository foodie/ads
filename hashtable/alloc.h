...
template < typename _Tp >
inline bool operator == (const alloc < _Tp > &, const alloc < _Tp > &) {
	return true;
}

template < typename _Tp, class _Alloc2 > 
inline bool operator == (const alloc <_Tp > &, const _Alloc2 &) {
	return false;
}
...
template <typename _Tp>
const bool alloc<_Tp>::recycle_space = false;
template <typename _Tp>
const bool alloc<_Tp>::thread_safe = true;
