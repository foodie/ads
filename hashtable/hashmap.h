...
public:
	...
	//typedef lib::pair<_Key, _Value> _Pair;	//将key value打包，作为底层容器的存储value
	/**
     ...
     * @brief hashtable别名
     */
    typedef hashtable<_Key, _Pair, _HashFun, _Equl, pair_first<_Pair>, _InnerAlloc> hash_type;
	/**
     * @brief 内部的hashtable
     */
...
public:
	...
	**/
	...
	value_type get(const key_type &k, value_type &def) const {
		_Pair *p = _ht.find(_ht._hashfun(k), k);
		return p != NULL ? p->second : def;
	}

	value_type get(const key_type &k, value_type &def) {
		_Pair *p = _ht.find(_ht._hashfun(k), k);
		return p != NULL ? p->second : def;
	}
	...
	**/
	...
	int serialization(_Archive &ar) {
		return lib::serialization(ar, _ht);
	}
	...
	int deserialization(_Archive &ar) {
		return lib::deserialization(ar, _ht);
	}
    /**
     * @brief 清空hashmap
     */
