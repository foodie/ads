#ifndef  __PHASHTABLE_H_
#define  __PHASHTABLE_H_
#include "hashtable.h"
#include "rwhashset.h"
...
public:
     ...
     * @brief 自身的别名
     */
    typedef phashtable<_Key, _Value, _HashFun, _Equl, _GetKey, _InnerAlloc> _Self;
	/**
     * @brief 内部使用的hashtable
     */
    typedef hashtable<_Key, _Value, _HashFun, _Equl, _GetKey, _InnerAlloc> _Base;
	/**
     * @brief 迭代器
     */
	...
	typedef rwhashset<pointer, xhash<pointer>, std::equal_to<pointer>, _InnerAlloc> _Set;
...
err:
	...
	void destruct_node(node_pointer node, size_t key) {
		...
		lib::bsl_destruct(&(_base._sp_alloc.getp(node)->val));
...
public:
     ...
     **/
    phashtable(size_t bitems, size_t rwlsize = 0,
        ...
        if (create(bitems,rwlsize,hf,eq) != 0) {
            ...
            std::cout << "create error when create phashtable with bitems " << bitems << std::endl;
        }
    }
	...
	**/
	...
	int deserialization(_Archive & ar) {
		size_t bitems = 0;
		if (lib::deserialization(ar, bitems) != 0) {
		...
		size_t __size = 0;
		if (lib::deserialization(ar, __size) != 0) {
			__BSL_ERROR("deserialization size error");
			return -1;
		}
		value_type val;
		for (size_t i=0; i<__size; ++i) {
			if (lib::deserialization(ar, val) != 0) {
...
public:
	...
	int _normal_set(size_t key, const key_type &k, const value_type &val, 
		...
		lib::bsl_construct(&(_base._sp_alloc.getp(node)->val), val);
		...
		{
			{
				...
				for (size_t i=0; i<_hashset->_ht._bitems; ++i) {
					...
					while (_hashset->_ht._bucket[i]) {
						...
						lib::bsl_destruct(&(_hashset->_ht._sp_alloc.getp(nd)->val));
