...
public:
	...
	int serialization(_Archive &ar) {
		if (lib::serialization(ar, first)) return -1;
		if (lib::serialization(ar, second)) return -1;
		return 0;
	}

	template <class _Archive>
	int deserialization(_Archive &ar) {
		if (lib::deserialization(ar, first)) return -1;
		if (lib::deserialization(ar, second)) return -1;
		return 0;
	}
};
#endif
