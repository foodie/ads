#ifndef  __PHASHMAP_H_
#define  __PHASHMAP_H_
...
template <class _Key, /*作为hashkey的类型*/
		  ...
		  * 但是在容器生命骑内不会释放申请的内存
		  *
		  * alloc<_Key>做内存分配器，可以在容器生命期内释放内存，
		  * 但是不能有效防止内存碎片
		  */
		 class _InnerAlloc = sample_alloc<alloc<_Key>, 256>
		>
class phashmap : public phashtable<_Key, std::pair<_Key, _Value>, _HashFun, _Equl, 
	lib::pair_first<std::pair<_Key, _Value> >, _InnerAlloc>
{
public:
	typedef std::pair<_Key, _Value> _Pair;
	typedef phashmap<_Key, _Value, _HashFun, _Equl, _InnerAlloc> _Self;
	typedef phashtable<_Key, _Pair, _HashFun, _Equl, lib::pair_first<_Pair>, _InnerAlloc> _Base;
...
public:
	...
	**/
	...
	value_type get(const key_type &k, const value_type &def) const {
		value_type val;
		if (_Base::get_map(k, &val) == lib::HASH_NOEXIST) {
			return def;
		}
		return val;
	}
	value_type get(const key_type &k, const value_type &def) {
		value_type val;
		if (_Base::get_map(k, &val) == lib::HASH_NOEXIST) {
			return def;
		}
	...
	value_type& operator[] (const key_type &k) {
        ...
        if (p == NULL) {
            value_type val;
            _Base::set(k, _Pair(k, val), 1);
            p = _Base::find(k);
        }
        value_type *v = p == NULL ? NULL : &p->second;
        return *v;
	}
};
