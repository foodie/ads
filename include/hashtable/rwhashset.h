#ifndef  __RWHASHSET_H_
#define  __RWHASHSET_H_
...
public:
	...
	typedef rwhashset<_Key, _HashFun, _Equl, _InnerAlloc> _Self;
	typedef hashtable<_Key, _Key, _HashFun, _Equl, param_select<_Key>, _InnerAlloc> hash_type;
...
public:
     ...
     * 不需调create
     **/
    rwhashset(size_t bitems, size_t lsize = 0,
        ...
        if (create(bitems,lsize,hf,eq) != 0) {
            ...
            std::cout << "create error when create rwhashset bitems " << bitems << std::endl;
        }
    }
	...
	int clear() {
		...
		for (size_t i=0; i<_ht._bitems; ++i) {
			...
			while (_ht._bucket[i]) {
				...
				lib::bsl_destruct(&_ht._sp_alloc.getp(nd)->val);
