#ifndef  __SAMPLE_ALLOC_H_
#define  __SAMPLE_ALLOC_H_
...
public:
	...
	typedef sample_alloc<_Alloc, _Items> self;
	...
	public:
		typedef sample_alloc<other_alloc, _Items> other;
	};
...
public:
	...
	sample_alloc(const sample_alloc<_Alloc, _Items> & ) { 
        create(); 
    }
...
template <class _Alloc, size_t _Items>
inline bool operator == (const sample_alloc < _Alloc, _Items > &, 
		const sample_alloc < _Alloc, _Items > &) {
	return false;
}
template <class _Alloc, size_t _Items, class _Alloc2>
inline bool operator == (const sample_alloc <_Alloc, _Items> &, const _Alloc2 &) {
	return false;
}
...
template <class _Alloc, size_t _Items>
const bool sample_alloc<_Alloc, _Items>::recycle_space = true;
template <class _Alloc, size_t _Items>
const bool sample_alloc<_Alloc, _Items>::thread_safe = false;
